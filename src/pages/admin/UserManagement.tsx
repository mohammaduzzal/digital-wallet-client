...
export default function UserManagement() {
    ...
    const { data: allUserData, isLoading: allUserLoading } = useGetAllUserQuery({ role: "USER",page:currentPage,limit })
        ...
        <div className="w-full max-w-7xl mx-auto px-5">
            ...
            <div className="border border-muted rounded-md">
                <Table>
                    ...
                    <TableBody>
                        {allUserData?.data?.map((item: { name: string, _id: string, wallet: { isBlocked: boolean,_id : string } }) => (
                            <TableRow>
                                ...
                                {item?.wallet?.isBlocked === true &&<TableCell><Button onClick={()=>handleUserUpdate(item.wallet._id,item.wallet.isBlocked)} variant="outline">Unblock</Button></TableCell>}

                                {item?.wallet?.isBlocked === false &&<TableCell><Button onClick={()=>handleUserUpdate(item.wallet._id,item.wallet.isBlocked)} variant="outline">Block</Button></TableCell>}
